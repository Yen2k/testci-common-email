...
public abstract class Email {
	 ...
	 * get property from system.properties. If still null, quit.
	 */
	protected String hostName;

	/**
	 * The port number of the mail server to connect to. Defaults to the standard
	 * port ( 25 ).
	 */
	...
	/**
	 * Address to which undeliverable mail should be sent. Because this is handled
	 * by JavaMail as a String property in the mail session, this property is of
	 * type <code>String</code> rather than <code>InternetAddress</code>.
	 */
	 ...
	 * login commands. Note that an appropriate trust store must configured so that
	 * the client will trust the server's certificate. Defaults to false.
	 */
	 ...
	 * the message anyway, reporting the partial failure with a SendFailedException.
	 * If set to false (the default), the message is not sent to any of the
	 * recipients if there is an invalid recipient address. Defaults to false.
	 */
	 ...
	 * before adding the message content.
	 *
	...
	/**
	 * Supply a mail Session object to use. Please note that passing a user name and
	 * password (in the case of mail authentication) will create a new mail session
	 ...
	 *
	 * @param jndiName name of JNDI resource (javax.mail.Session type), resource if
	 *                 searched in java:comp/env if name does not start with "java:"
	...
	public String sendMimeMessage() throws EmailException {
		...
		try {
			...
			String msg = "Sending the email to the following server failed : " + this.getHostName() + ":"
					+ this.getSmtpPort();
	...
	/**
	 * Returns the internal MimeMessage. Please not that the MimeMessage is build by
	 * the buildMimeMessage() method.
	 *
	...
	/**
	 * Factory method to create a customized MimeMessage which can be implemented by
	 * a derived class, e.g. to set the message id.
	 *
	 * @param aSession mail session to be used
	...
	/**
	 * When a mail session is already initialized setting the session properties has
	 * no effect. In order to flag the problem throw an IllegalStateException.
	 *
	 * @throws IllegalStateException when the mail session is already initialized
	 */
